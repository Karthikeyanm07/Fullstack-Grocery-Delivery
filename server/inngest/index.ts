...
// 1. Low Stock Alert t0 Admin Email
const checkLowStock = inngest.createFunction(
	...
	async ({ event, step }) => {
		...
		if (
			...
			return {
				skipped: true,
				reason: "Stock OK or product not found",
				stock: product?.stock,
			};
		}
		...
		await step.run("send-low-stock-email", async () => {
			...
			// Extract plain email strings from the { email: string }[] objects
			// so nodemailer receives string[] not { email: string }[]
...
const sendMonthlyOffers = inngest.createFunction(
	...
	async ({  step }) => {
		...
		if (deals.length === 0 || users.length === 0) {
			return {skipped: true, reason: "No deals or users found"};
		}

		let sentCount = 0;

		// Send in batches of 10 to avoid overloading email servers 
		const batchSize = 10;
...
// Auto-Assign Rider after 5 minutes of order placement
const autoAssignRider = inngest.createFunction(
	...
	async ({ event, step }) => {
		...
		// Wait 5 minutes before auto-assigning rider
		await step.sleep("wait-5-minutes", "5 minutes");

		const result = await step.run("auto-assign-rider", async () => {
			const order = await prisma.order.findUnique({
				where: {id: orderId},
			})

			// Skip if order is already assigned or not placed
			if(!order ){
				return {skipped: true, reason: "Order not found or already assigned"};
			}
			...
			const bustRidersId = busyOrders.map((o) => o.deliveryPartnerId).filter((id): id is string => id !== null);

			const availableRiders = await prisma.deliveryPartner.findFirst({
				where: {
					id: {notIn: bustRidersId as string[]},
					isActive: true,
				},
			})
			...
			return {assigned: true, riderId: availableRiders.id, riderName: availableRiders.name, otp: otp};
		})
		return result;
	},
